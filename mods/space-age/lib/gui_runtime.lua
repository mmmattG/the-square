...
local function get_player_planet_state(player)
  if not (player and player.valid and player.surface) then
    return planet_state.get_planet("nauvis")
  end

  return planet_state.get_planet_for_surface(player.surface.name) or planet_state.get_planet("nauvis")
end

local function build_status_lines(player)
  local bootstrap = storage.bootstrap
  local lines = {}
  local current_planet_state = get_player_planet_state(player)
  local current_planet_key = current_planet_state and current_planet_state.planet_key or "nauvis"
  ...
  lines[#lines + 1] = "Planet: " .. current_planet_key
  ...
  lines[#lines + 1] = "Expansion points: " .. (current_planet_state and current_planet_state.expansion_points or 0)

  return lines
end

local function build_debug_lines(player)
  local lines = build_status_lines(player)
...
function gui_runtime.refresh_debug_gui(player)
  ...
  for _, line in ipairs(build_debug_lines(player)) do
    frame.add({
      type = "label",
      caption = line
    })
...
function gui_runtime.refresh_shop_gui(player, anchor_runtime)
  ...
  local bootstrap = storage.bootstrap
  local current_planet_state = get_player_planet_state(player)
  local current_points = current_planet_state and current_planet_state.expansion_points or 0
  ...
  frame.add({
    type = "label",
    caption = {"gui.fes-shop-points", current_points}
  })
  frame.add({
    type = "label",
    caption = {
  ...
  for _, definition in ipairs(defs.INPUT_DEFINITIONS) do
    ...
    button.enabled = can_purchase and current_points >= defs.get_line_purchase_cost()
