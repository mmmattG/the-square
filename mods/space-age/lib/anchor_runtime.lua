...
local function try_unlock_trigger_technologies(anchor, force, debug_recipient)
  local definition = anchor and defs.get_input_definition(anchor.resource) or nil
  local trigger_technologies = definition and definition.trigger_technologies or nil

  if not trigger_technologies then
    return
  end

  for _, technology_name in ipairs(trigger_technologies) do
    try_unlock_ingress_technology(
      anchor,
      force,
      debug_recipient,
      anchor.resource,
      technology_name,
      defs.format_resource_name(anchor.resource)
    )
  end
end
...
local function spend_expansion_points(player, amount)
  local surface_name = player and player.valid and player.surface and player.surface.name or nil
  local planet_key = defs.get_planet_key_for_surface_name(surface_name) or "nauvis"

  if planet_key == "nauvis" and storage.bootstrap then
    planet_state.sync_planet_from_bootstrap("nauvis", storage.bootstrap)
  end

  local spent, remaining_points = planet_state.try_spend_expansion_points(planet_key, amount)
  ...
  if planet_key == "nauvis" and storage.bootstrap then
    storage.bootstrap.expansion_points = remaining_points
  end
...
function anchor_runtime.purchase_managed_line_for_resource(player, resource)
  ...
  local spent_points, remaining_points = spend_expansion_points(player, line_purchase_cost)

  if not spent_points then
    if player and player.valid then
...
local function handle_managed_anchor_built(entity, actor, gui_runtime)
  ...
  if assign_anchor_position(anchor, side, anchor_position) then
    ...
    try_unlock_trigger_technologies(anchor, entity.force, actor)
  end
...
function anchor_runtime.handle_managed_anchor_slot_click(player)
  ...
  if assign_anchor_position(anchor, side, tile_position) then
    ...
    try_unlock_trigger_technologies(anchor, player.force, player)
  end
