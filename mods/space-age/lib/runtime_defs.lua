...
runtime_defs.INGRESS_RESEARCH_DEFINITIONS = {
  ...
  {
    technology_name = "fes-ingress-green",
...
runtime_defs.INPUT_DEFINITIONS = planet_catalog.get_input_definitions("nauvis")
runtime_defs.OUTPUT_DEFINITIONS = planet_catalog.get_output_definitions("nauvis")
...
function runtime_defs.get_expansion_research_band_for_level(level)
  return planet_catalog.get_expansion_research_band("nauvis", level)
end

function runtime_defs.get_planet_key_for_surface_name(surface_name)
  if surface_name == runtime_defs.SURFACE_NAME then
    return "nauvis"
  end

  local planet = planet_catalog.get_planet_for_surface(surface_name)
  return planet and planet.key or nil
end
