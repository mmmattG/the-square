...
local item_ingress_belt_tiers = {
  ...
  {key = "green", prototype_name = "turbo-underground-belt"}
}
...
local ingress_research_definitions = {
  ...
  {
    name = "fes-ingress-green",
    icon = "__space-age__/graphics/icons/turbo-transport-belt.png",
    prerequisite_technology_name = "turbo-transport-belt",
    previous_ingress_technology_name = "fes-ingress-blue",
    localised_name = {"technology-name.fes-ingress-green"},
    localised_description = {"technology-description.fes-ingress-green"},
    effect_description = {"technology-effect.fes-ingress-green"}
  }
}
...
local function build_square_expansion_technology(definition)
  ...
  return {
    ...
    localised_name = definition.localised_name or {"technology-name.fes-square-expansion"},
...
for _, planet in ipairs(planet_catalog.get_all()) do
  local starting_square_size = planet.starting_square_size

  for level = 1, expansion_research.FINAL_FINITE_LEVEL do
    local band = planet_catalog.get_expansion_research_band(planet.key, level)
    local prerequisites = nil

    if level > 1 then
      prerequisites = {expansion_research.get_technology_name(planet.key, level - 1)}
    elseif planet.unlock_technology_name then
      prerequisites = {planet.unlock_technology_name}
    end

    prototypes[#prototypes + 1] = build_square_expansion_technology({
      name = expansion_research.get_technology_name(planet.key, level),
      localised_name = planet.display_name .. " square expansion",
      order = string.format("c-a[square-expansion]-%s-%04d", planet.key, level),
      prerequisites = prerequisites,
  ...
  prototypes[#prototypes + 1] = build_square_expansion_technology({
    name = expansion_research.get_technology_name(planet.key, expansion_research.INFINITE_START_LEVEL),
    localised_name = planet.display_name .. " square expansion",
    order = string.format("c-a[square-expansion]-%s-%04d", planet.key, expansion_research.INFINITE_START_LEVEL),
    prerequisites = {expansion_research.get_technology_name(planet.key, expansion_research.INFINITE_START_LEVEL - 1)},
    ingredients = planet_catalog.get_expansion_research_band(planet.key, expansion_research.INFINITE_START_LEVEL).ingredients,
