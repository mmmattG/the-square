...
This repo is the separate Space Age-only variant of The Square. It requires the Space Age expansion and is being split from the vanilla mod so the original project can remain DLC-free.

The current implementation is in transition from the vanilla mod's single bootstrap surface toward a planet-scoped model with one square per supported planet. The first slice in this repo introduces a planet catalog/state layer and planet-specific square expansion research generation. Runtime square growth is still only wired through Nauvis while the rest of the planet runtime is being refactored.
