...
run_test("green double lane sustains turbo-belt cadence", function()
  local simulation = simulate_ticks({4, 4}, 16)

  assert_equal(simulation.totals[1], 8, "the first lane should sustain green-belt cadence")
  assert_equal(simulation.totals[2], 8, "the second lane should sustain green-belt cadence")

  for tick = 1, 16 do
    assert_at_most(
      simulation.per_tick_emissions[tick][1],
      1,
      "green belts should pace inserts rather than depend on same-tick stacking"
    )
    assert_at_most(
      simulation.per_tick_emissions[tick][2],
      1,
